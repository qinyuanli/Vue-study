...
## 配置项 props
     ...
     备注：props是只读的，Vue底层会监视你对props的修改，就会发出警告，若业务需求确实需要修改，那么请复制props的内容到data中一份，然后去修改data中的数据。
...
1.组件话编码流程：
（1）拆分静态组件：组件要按照功能点拆分，命名不要与 html 元素冲突（最好是双驼峰）
...
3.使用 v-model 时要切记：v-model 绑定的值不能是 props 传过来的值，因为 props 只读（不可修改）

4.props 传过来的值若是对象类型的值，修改对象中的属性 Vue 不会报错，但不推荐这样做
...
## 组件的自定义事件
    ...
    3.绑定自定义事件：
      1.第一种方式，在父组件中借助子组件的标签：<Demo @atguigu = "test"></Demo> 或 <Demo v-on:atguigu="test"></Demo>
      ...
      4.触发自定义事件：this.$emit('atguigu',数据params)

      5.解绑自定义事件：this.$off('atguigu')

      6.组件上也可以绑定原生DOM事件，vue2中需要使用native修饰符（vue3中已弃用native）
